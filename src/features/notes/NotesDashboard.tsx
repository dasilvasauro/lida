...
// === PARSER MARKDOWN (Text -> HTML) ===
const parseMarkdown = (text: string) => {
  if (!text) return '';
  let html = text;

  // Blocos de Código (Multiline) - Escapado para evitar erros de compilação
  ...
  // Quebras de Linha (Respeitando blocos de código)
  const segments = html.split(/(<pre[\s\S]*?<\/pre>)/);
  html = segments.map((seg, i) => {
    if (i % 2 === 1) return seg; // Ignora o conteúdo dentro do <pre>
    return seg.replace(/\n/g, '<br/>');
  }).join('');
...
// === REVERSER HTML -> MARKDOWN ===
const htmlToMarkdown = (html: string) => {
  if (!html) return '';
  let md = html;

  // Converter divs e parágrafos para quebras de linha
  ...
  // Formatação (O <u> vai ser preservado intacto)
  ...
  // Limpar invólucros de listas
  md = md.replace(/<ul[^>]*>/gi, '\n'); md = md.replace(/<\/ul>/gi, '\n');
  md = md.replace(/<ol[^>]*>/gi, '\n'); md = md.replace(/<\/ol>/gi, '\n');

  // Remover qualquer outro HTML restante, MAS PROTEGER A TAG <u>
  md = md.replace(/<(?!u|\/u)[^>]+>/g, '');
    ...
    <style>{`
      ...
      {view === 'editor' && activeNote && activeNotebook && (
        ...
        {isFullscreen && (
          ...
          <div className="w-px h-5 bg-zinc-300 dark:bg-zinc-700" />
          <button onClick={() => { setIsEditing(!isEditing); setShowInNoteSearch(false); setNoteSearchQuery(''); }} className={`flex items-center gap-2 px-3 py-1 rounded-full text-[10px] font-black uppercase tracking-wider transition-colors ${isEditing ? 'bg-blue-500 text-white' : 'bg-transparent border border-zinc-400 dark:border-zinc-600 opacity-80 hover:opacity-100'}`}>
