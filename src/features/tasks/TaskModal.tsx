              ...
              <div className="p-6 overflow-y-auto flex-1 space-y-8 scrollbar-hide">
                    ...
                    <AnimatePresence mode="popLayout">
                      {recurrenceType !== 'none' && (
                        <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: 'auto', opacity: 1 }} exit={{ height: 0, opacity: 0 }} className="space-y-3 overflow-hidden">
                          ...
                          {(recurrenceType === 'monthly' || recurrenceType === 'yearly') && (<div className="bg-blue-500/10 border border-blue-500/20 text-blue-600 dark:text-blue-400 p-3 rounded-xl text-[11px] flex items-start gap-2 italic"><Info size={14} className="mt-0.5 shrink-0" /><p>Repetirá com base no dia da <b>"Data Limite"</b> abaixo.</p></div>)}
                        </motion.div>
                      )}
                    </AnimatePresence>
                  </div>
                ...
                <div>
                  ...
                  <div className="flex gap-2 overflow-x-auto scrollbar-hide pb-2">
                    {folders.map((f) => (<button key={f.id} onClick={(e) => { e.preventDefault(); setFolderId(f.id); }} className={`px-4 py-2 rounded-xl border font-bold text-xs whitespace-nowrap ${folderId === f.id ? 'bg-zinc-900 text-white dark:bg-zinc-100 dark:text-black border-transparent shadow-md' : 'bg-transparent border-zinc-200 dark:border-zinc-800 hover:bg-zinc-100 dark:hover:bg-zinc-800 text-zinc-500'}`}>{f.name}</button>))}
                    {isCreatingFolder ? (<div className="flex items-center gap-1 bg-zinc-100 dark:bg-zinc-800 rounded-xl px-2 border border-zinc-200 dark:border-zinc-700 shadow-inner"><input autoFocus type="text" maxLength={30} value={newFolderName} onChange={(e) => setNewFolderName(e.target.value)} onKeyDown={(e) => { if(e.key === 'Enter'){ e.preventDefault(); handleCreateFolder(); } }} placeholder="Nome" className="bg-transparent text-xs font-bold outline-none w-28 px-2" /><button onClick={(e) => { e.preventDefault(); handleCreateFolder(); }} className="p-1.5 text-emerald-500"><Check size={14} /></button><button onClick={(e) => { e.preventDefault(); setIsCreatingFolder(false); }} className="p-1.5 text-red-500"><X size={14} /></button></div>) : (<button onClick={(e) => { e.preventDefault(); setIsCreatingFolder(true); }} className="px-4 py-2 rounded-xl text-xs font-bold whitespace-nowrap border border-dashed border-zinc-300 dark:border-zinc-700 text-zinc-500 hover:bg-zinc-100 dark:hover:bg-zinc-800">+ Nova Pasta</button>)}
                  </div>
                </div>
        ...
        {showConfirmClose && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-[60] bg-black/80 flex items-center justify-center p-6 backdrop-blur-sm">
            <motion.div initial={{ scale: 0.95 }} animate={{ scale: 1 }} className="bg-white dark:bg-zinc-900 w-full max-w-sm rounded-3xl p-6 shadow-2xl border border-zinc-200 dark:border-zinc-800 text-center">
              ...
              <p className="text-zinc-500 dark:text-zinc-400 mb-6 text-sm">Você tem campos preenchidos. Se fechar agora, perderá essas informações.</p>
