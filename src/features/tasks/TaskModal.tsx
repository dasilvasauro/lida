...
import { X, Zap, Target, Timer, Gift, CheckCircle2, Calendar, Clock, Plus, RotateCcw, Info, Lock, Check } from 'lucide-react';
...
export const TaskModal = ({ isOpen, onClose, taskToEdit, onSuccess }: TaskModalProps) => {
  ...
  // FIX: Ignora tareas que ya fueron concluidas o archivadas en el bloqueo de P0/P1
  ...
  const [title, setTitle] = useState(''); const [description, setDescription] = useState(''); const [priority, setPriority] = useState<Priority>('P4'); const [type, setType] = useState<TaskType>('normal'); const [folderId, setFolderId] = useState<string>('default');
  const [isCreatingFolder, setIsCreatingFolder] = useState(false); const [newFolderName, setNewFolderName] = useState('');
  const [deadlineDate, setDeadlineDate] = useState<string>(''); const [deadlineTime, setDeadlineTime] = useState<string>(''); const [startDate, setStartDate] = useState<string>(''); const [endDate, setEndDate] = useState<string>(''); const [duration, setDuration] = useState<number>(30);
  const [subtaskInput, setSubtaskInput] = useState(''); const [subtasks, setSubtasks] = useState<{ id: string; title: string; completed: boolean }[]>([]);
  const [recurrenceType, setRecurrenceType] = useState<'none' | 'weekly' | 'monthly' | 'yearly'>('none'); const [selectedWeekdays, setSelectedWeekdays] = useState<number[]>([]);
  const [showDatePicker, setShowDatePicker] = useState(false); const [showTimePicker, setShowTimePicker] = useState(false); const [activeDateField, setActiveDateField] = useState<'deadline' | 'start' | 'end' | null>(null);

  const canBeRecurrent = !['sprint', 'daily_challenge', 'bonus'].includes(type);

  useEffect(() => {
    if (taskToEdit && isOpen) {
      setTitle(taskToEdit.title); setDescription(taskToEdit.description || ''); setPriority(taskToEdit.priority); setType(taskToEdit.type); setFolderId(taskToEdit.folderId || 'default'); setDeadlineDate(taskToEdit.deadlineDate || ''); setDeadlineTime(taskToEdit.deadlineTime || '');
      ...
      setSubtasks(taskToEdit.subtasks || []); setDuration(taskToEdit.duration || 30); setRecurrenceType(taskToEdit.recurrence?.type || 'none'); setSelectedWeekdays(taskToEdit.recurrence?.weekdays || []);
    } else if (isOpen) {
      setTitle(''); setDescription(''); setPriority('P4'); setType('normal'); setFolderId('default'); setDeadlineDate(''); setDeadlineTime(''); setStartDate(''); setEndDate(''); setSubtasks([]); setDuration(30); setRecurrenceType('none'); setSelectedWeekdays([]); setShowDatePicker(false); setShowTimePicker(false); setIsCreatingFolder(false); setNewFolderName('');
    }
  }, [taskToEdit, isOpen]);
  ...
  const handleSave = () => {
    if (!title.trim()) return; if (type === 'sprint' && subtasks.length === 0) return;
    if (!taskToEdit) { if (type === 'bonus') useItem('bonusTask'); if (priority === 'P0' && countP0 >= 1) useItem('extraP0'); if (priority === 'P1' && countP1 >= 2) useItem('extraP1'); }
    ...
    const taskData: Partial<Task> = {
      ...
      deadlineTime: deadlineTime || undefined, duration: type === 'time' ? duration : undefined,
    ...
    if (taskToEdit) { updateTask(taskToEdit.id, taskData); onSuccess?.('Tarea actualizada con éxito!'); } 
    else { addTask({ id: uuidv4(), createdAt: Date.now(), isCompleted: false, ...taskData } as Task); onSuccess?.('Tarea creada con éxito!'); }
    onClose();
  };
    ...
    <AnimatePresence>
      {isOpen && (
        <>
          ...
          <motion.div initial={{ y: '100%' }} animate={{ y: 0 }} exit={{ y: '100%' }} transition={{ type: 'tween', duration: 0.3, ease: 'easeOut' }} className="fixed bottom-0 left-0 right-0 md:bottom-6 max-w-md mx-auto bg-zinc-50 dark:bg-zinc-900 rounded-t-3xl md:rounded-3xl shadow-2xl z-50 overflow-hidden flex flex-col max-h-[90vh]">
            <div className="flex justify-between items-center p-6 border-b border-zinc-200 dark:border-zinc-800"><h3 className="text-xl font-bold">{taskToEdit ? 'Editar Tarefa' : 'Nova Tarefa'}</h3><button onClick={onClose} className="p-2 rounded-full bg-zinc-200 dark:bg-zinc-800 hover:bg-zinc-300 dark:hover:bg-zinc-700 transition-colors"><X size={20} /></button></div>
            <div className="p-6 overflow-y-auto flex-1 space-y-8 scrollbar-hide">
              ...
              <div>
                ...
                <div className="grid grid-cols-3 gap-2">
                  {taskTypes.map((t) => {
                    ...
                    return (
                      <button key={t.id} onClick={(e) => { e.preventDefault(); if (isDisabled) return; setType(t.id); setShowDatePicker(false); setShowTimePicker(false); }} disabled={isDisabled} className={`relative flex flex-col items-center justify-center py-2 rounded-xl border transition-all ${isSelected ? 'bg-zinc-200 border-zinc-400 dark:bg-zinc-800 dark:border-zinc-500 shadow-inner' : isDisabled ? 'opacity-40 cursor-not-allowed grayscale border-zinc-200 dark:border-zinc-800 bg-zinc-100/50 dark:bg-zinc-900/50' : 'bg-transparent border-zinc-200 dark:border-zinc-800 hover:bg-zinc-100 dark:hover:bg-zinc-800'}`}>
              ...
              {type === 'sprint' && (
                <div className="flex gap-4">
                  <div className="flex-1 space-y-3"><span className="text-xs uppercase tracking-widest text-zinc-500 block font-bold">Inicio</span><button onClick={(e) => { e.preventDefault(); setActiveDateField('start'); setShowDatePicker(true); setShowTimePicker(false); }} className={`w-full p-3 rounded-lg border text-sm transition-all ${startDate ? 'border-zinc-900 dark:border-zinc-100 font-bold' : 'border-zinc-200 dark:border-zinc-800 text-zinc-400'}`}>{startDate ? format(new Date(startDate + 'T12:00:00'), "dd 'de' MMM", { locale: ptBR }) : 'Data Inicial'}</button></div>
                  <div className="flex-1 space-y-3"><span className="text-xs uppercase tracking-widest text-zinc-500 block font-bold">Fin</span><button onClick={(e) => { e.preventDefault(); setActiveDateField('end'); setShowDatePicker(true); setShowTimePicker(false); }} className={`w-full p-3 rounded-lg border text-sm transition-all ${endDate ? 'border-zinc-900 dark:border-zinc-100 font-bold' : 'border-zinc-200 dark:border-zinc-800 text-zinc-400'}`}>{endDate ? format(new Date(endDate + 'T12:00:00'), "dd 'de' MMM", { locale: ptBR }) : 'Data Final'}</button></div>
                </div>
              )}
              ...
              {type === 'time' && (
                <div className="space-y-3"><span className="text-xs uppercase tracking-widest text-zinc-500 block font-bold flex items-center gap-1"><Timer size={14} /> Duración: <span className="text-blue-500 ml-1">{duration} min</span></span><input type="range" min="5" max="120" step="5" value={duration} onChange={(e) => setDuration(Number(e.target.value))} className="w-full accent-blue-500" /></div>
              )}
              ...
              <div>
                ...
                <div className="flex gap-2">
                  {(['P0', 'P1', 'P2', 'P3', 'P4'] as Priority[]).map((p) => {
                    ...
                    return (<button key={p} onClick={(e) => { e.preventDefault(); if (!isLocked) setPriority(p); }} disabled={isLocked} className={`relative flex-1 py-2 rounded-lg border font-bold transition-all ${colorClass} ${isLocked ? 'opacity-40 grayscale cursor-not-allowed' : ''}`}>{p}{isLocked && <Lock size={10} className="absolute top-1 right-1 opacity-50" />}</button>)
                  })}
                </div>
              </div>
