...
export const GoogleConnectModal = ({ isOpen, onClose, onSuccess }: { isOpen: boolean; onClose: () => void; onSuccess: () => void }) => {
  ...
  const handleConnect = async () => {
    ...
    try {
      ...
      console.warn("Pop-up bloqueado, tentando via redirecionamento de segurança...", e);
      try {
  ...
  return (
    <AnimatePresence>
      <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-[100] bg-black/80 flex items-center justify-center p-4">
        <div className="bg-white dark:bg-zinc-900 w-full max-w-md rounded-3xl overflow-hidden shadow-2xl relative p-8 text-zinc-900 dark:text-zinc-100">
          ...
          {step === 'intro' && (
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="flex flex-col items-center text-center">
              ...
              <button onClick={handleConnect} className="w-full flex items-center justify-center gap-3 bg-zinc-900 text-white dark:bg-zinc-100 dark:text-black py-4 rounded-xl font-bold hover:scale-105 transition-transform cursor-pointer">
          ...
          {step === 'conflict' && (
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="flex flex-col items-center text-center">
              ...
              <div className="w-full space-y-3">
                <button onClick={() => setStep('enter_pin')} className="w-full p-4 border border-zinc-200 dark:border-zinc-800 rounded-xl flex items-center justify-between group hover:border-blue-500 hover:bg-blue-500/5 transition-all cursor-pointer">
                ...
                <button onClick={() => setStep('create_pin')} className="w-full p-4 border border-zinc-200 dark:border-zinc-800 rounded-xl flex items-center justify-between group hover:border-emerald-500 hover:bg-emerald-500/5 transition-all cursor-pointer">
                  <div className="text-left">
          ...
          {step === 'enter_pin' && (
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="flex flex-col items-center text-center">
              ...
              {error && <p className="text-red-500 text-sm font-bold mb-4">{error}</p>}
              <button onClick={handlePullCloud} className="w-full py-4 bg-blue-500 text-white rounded-xl font-bold cursor-pointer">Restaurar Dados</button>
            </motion.div>
          )}
          ...
          {step === 'create_pin' && (
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="flex flex-col items-center text-center">
              ...
              {error && <p className="text-red-500 text-sm font-bold mb-4">{error}</p>}
              <button onClick={handlePushLocal} className="w-full py-4 bg-emerald-500 text-white rounded-xl font-bold cursor-pointer">Sobrescrever Nuvem</button>
            </motion.div>
          )}
