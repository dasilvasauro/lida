...
import { Star, Coins, Flame, CheckCircle2, TrendingUp, Eye, Medal, Info, Activity, RefreshCw, Settings } from 'lucide-react';
...
export const ProfileDashboard = () => {
  ...
  const getMoodColor = (mood: Mood | undefined) => {
    switch(mood) {
      case 'radiant': return 'bg-emerald-500 shadow-[0_0_8px_rgba(16,185,129,0.4)]'; case 'happy': return 'bg-teal-400'; case 'normal': return 'bg-yellow-400'; case 'annoyed': return 'bg-orange-500'; case 'disappointed': return 'bg-red-500'; default: return 'bg-zinc-200 dark:bg-zinc-800/50';
    }
  };
      ...
      <div className="max-w-4xl mx-auto px-6 md:px-8 pt-12 space-y-8">
        ...
        <header className="pb-6 border-b border-zinc-100 dark:border-zinc-900 flex justify-between items-start">
          ...
          <div className="flex items-center gap-2">
            {!isLocalMode && (
              <button onClick={handleForceSync} disabled={isSyncing} className="p-2 px-4 rounded-full bg-zinc-100 dark:bg-zinc-800 hover:bg-zinc-200 dark:hover:bg-zinc-700 transition-colors text-xs font-bold text-zinc-500 dark:text-zinc-400 flex items-center gap-2 disabled:opacity-50">
        ...
        <button onClick={() => setIsVisionOpen(true)} className="w-full relative p-6 rounded-3xl border border-zinc-200 dark:border-zinc-800 bg-zinc-100 dark:bg-zinc-900/50 flex flex-col md:flex-row items-start md:items-center gap-6 overflow-hidden hover:border-blue-500/50 transition-colors group shadow-sm text-left">
          ...
          <div className="flex-1">
            <div className="flex flex-wrap items-center gap-3 mb-1"><h3 className="text-xl font-black text-zinc-700 dark:text-zinc-300 group-hover:text-zinc-900 dark:group-hover:text-zinc-100 transition-colors tracking-tight">Módulo Visão</h3><span className="px-3 py-1 bg-blue-500/10 text-blue-500 text-[10px] font-black uppercase tracking-widest rounded-full">Acessar</span></div>
            <p className="text-sm text-zinc-500 dark:text-zinc-400 font-medium">Seu plano mestre de longo prazo. Metas, características e o propósito da sua jornada.</p>
          </div>
        </button>
