...
import { X, Hash, AtSign, Send, MessageSquareText, Search, Calendar as CalendarIcon, MoreVertical, Archive, ArchiveRestore, Edit2, Trash2, CornerDownRight, PaintBucket, AlertTriangle, AlertCircle, CheckCircle2, Zap, Footprints, Timer, Gift, Repeat, Check, Clock } from 'lucide-react';
...
const LinkedTaskCard = ({ taskId }: { taskId: string }) => {
    ...
    return (
        <div className={`mt-3 p-3 rounded-xl border border-zinc-200 dark:border-zinc-800 bg-white dark:bg-black/50 flex flex-col gap-2 ${task.isCompleted ? 'opacity-60' : ''}`}>
            <div className="flex items-center gap-2">
               <div className={`w-5 h-5 rounded-full border-2 flex items-center justify-center shrink-0 ${task.isCompleted ? 'bg-zinc-900 border-zinc-900 text-white dark:bg-zinc-100 dark:border-zinc-100 dark:text-black' : 'border-zinc-400 dark:border-zinc-500'}`}>
                  {task.isCompleted && <Check size={12} strokeWidth={3} />}
               </div>
               <span className={`text-sm font-bold truncate ${task.isCompleted ? 'line-through opacity-70' : 'text-zinc-900 dark:text-zinc-100'}`}>{task.title}</span>
            </div>
            <div className="flex flex-wrap items-center gap-2 pl-7">
                ...
                <span className="flex items-center gap-1 text-[10px] font-bold uppercase tracking-widest text-zinc-500"><Icon size={12}/> {task.type.replace('_', ' ')}</span>
                {task.deadlineDate && <span className="flex items-center gap-1 text-[10px] font-bold uppercase tracking-widest text-zinc-500"><CalendarIcon size={12} />{format(new Date(task.deadlineDate + 'T12:00:00'), "dd/MM", { locale: ptBR })}</span>}
                {task.deadlineTime && <span className="flex items-center gap-1 text-[10px] font-bold uppercase tracking-widest text-zinc-500"><Clock size={12} />{task.deadlineTime}</span>}
            </div>
        </div>
    );
};
...
export const FeedDashboard = ({ isOpen, onClose, focusInputSignal }: { isOpen: boolean, onClose: () => void, focusInputSignal: number }) => {
  ...
  const FeedSummaryCard = ({ feed }: { feed: Feed }) => {
      ...
      return (
          <button onClick={() => { setActiveChannelId(feed.channelId); setActiveFeedId(feed.id); }}
             className={`flex flex-col p-5 rounded-2xl border text-left transition-transform hover:scale-[1.02] bg-white dark:bg-zinc-900/50 hover:border-current shadow-sm ${style.text} ${style.border}`}>
             ...
             {latestEntry ? (
                 <p className="text-sm text-zinc-500 dark:text-zinc-400 line-clamp-3 leading-relaxed flex-1 w-full">{latestEntry.content}</p>
             ) : (
                 <p className="text-sm text-zinc-400 italic flex-1 w-full">Nenhum registro ativo.</p>
             )}
             <div className="mt-4 pt-3 border-t border-zinc-100 dark:border-zinc-800/50 w-full">
            ...
            <div className="flex-1 overflow-y-auto scrollbar-hide p-4 space-y-6">
                <div>
                   ...
                   <div className="space-y-1">
                      <button onClick={() => { setActiveChannelId(null); setActiveFeedId(null); }} className={`w-full text-left px-3 py-2 rounded-xl text-sm font-bold transition-colors ${!activeChannelId ? 'bg-zinc-900 text-white dark:bg-white dark:text-black shadow-md' : 'text-zinc-600 dark:text-zinc-400 hover:bg-zinc-100 dark:hover:bg-zinc-900'}`}>
                         Todos os Canais
                      </button>
                      {channels.map(c => {
                                ...
                                return (
                                    <React.Fragment key={group.parent.id}>
                                        ...
                                        <div className="group/parent flex flex-col gap-2 relative">
                                            ...
                                            <div className={`flex gap-4 relative z-10 p-4 rounded-2xl transition-colors ${editingEntry?.id === group.parent.id || replyingTo?.id === group.parent.id ? 'bg-blue-500/5 ring-1 ring-blue-500/30' : 'bg-white dark:bg-zinc-900/50 border border-zinc-200 dark:border-zinc-800/80 shadow-sm hover:border-zinc-300 dark:hover:border-zinc-700'}`}>
                                            ...
                                            {group.replies.map(reply => (
                                                <div key={reply.id} className={`group/reply ml-8 md:ml-12 flex gap-4 relative z-10 p-4 rounded-2xl transition-colors ${editingEntry?.id === reply.id ? 'bg-blue-500/5 ring-1 ring-blue-500/30' : 'bg-white dark:bg-zinc-900/50 border border-zinc-200 dark:border-zinc-800/80 shadow-sm hover:border-zinc-300 dark:hover:border-zinc-700'}`}>
                                                    ...
                                                    <div className="flex-1 min-w-0 pt-0.5">
                                                        ...
                                                        {reply.linkedTaskId && (
                                                            <LinkedTaskCard taskId={reply.linkedTaskId} />
                                                        )}
                                                    </div>
                    ...
                    <div className="space-y-6">
                        ...
                        {activeFeeds.length === 0 ? (
                           ...
                           <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
                               {activeFeeds.map(f => <FeedSummaryCard key={f.id} feed={f} />)}
                           </div>
                        )}
                    </div>
                ) : (
                        ...
                        {channels.map(c => {
                            ...
                            return (
                                <div key={c.id}>
                                    ...
                                    <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
                                        {cFeeds.map(f => <FeedSummaryCard key={f.id} feed={f} />)}
                                    </div>
                                </div>
                            )
                        })}
                ...
                <div className="p-4 bg-white/80 dark:bg-black/80 backdrop-blur-md border-t border-zinc-200 dark:border-zinc-900 relative">
                    ...
                    <div className="flex items-end gap-2 relative">
                        <textarea
                            ...
                            rows={1}
                        />
                        <button onClick={handleSend} disabled={!inputText.trim()} className="p-4 bg-blue-600 text-white rounded-2xl font-bold disabled:opacity-50 hover:bg-blue-500 transition-colors shadow-lg shadow-blue-600/20 flex items-center justify-center shrink-0">
