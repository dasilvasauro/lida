...
export const RewardToast = ({ breakdown }: RewardToastProps) => {
    ...
    useEffect(() => {
        ...
        setVisibleSteps([0]); // Mostra a Base imediatamente
        ...
        let delay = 800; // Tempo entre a aparição de cada bônus
    ...
    return (
        ...
        {breakdown && (
            <motion.div
                ...
                className="fixed bottom-28 md:bottom-12 left-1/2 md:left-auto md:right-32 -translate-x-1/2 md:translate-x-0 w-[90%] max-w-sm bg-zinc-900 text-zinc-100 dark:bg-white dark:text-zinc-900 p-5 rounded-3xl shadow-[0_20px_50px_rgba(0,0,0,0.3)] z-50 border border-zinc-800 dark:border-zinc-200"
            >
                {breakdown.isFailed ? (
                    ...
                    <div className="flex flex-col">
                        <div className="flex items-center gap-2 mb-4 pb-3 border-b border-zinc-800 dark:border-zinc-200">
                        ...
                        <div className="space-y-3 mb-5 text-sm font-bold">
                            {visibleSteps.includes(0) && (
                                <motion.div initial={{opacity:0, x:-10}} animate={{opacity:1, x:0}} className="flex justify-between items-center text-zinc-400 dark:text-zinc-500">
                            ...
                            {visibleSteps.includes(1) && (
                                <motion.div initial={{opacity:0, x:-10}} animate={{opacity:1, x:0}} className="flex justify-between items-center text-blue-400 dark:text-blue-600">
                            ...
                            {visibleSteps.includes(2) && (
                                <motion.div initial={{opacity:0, x:-10}} animate={{opacity:1, x:0}} className="flex justify-between items-center text-purple-400 dark:text-purple-600">
                            ...
                            {visibleSteps.includes(3) && (
                                <motion.div initial={{opacity:0, x:-10}} animate={{opacity:1, x:0}} className="flex justify-between items-center text-orange-400 dark:text-orange-600">
                        ...
                        <div className="flex justify-between items-center pt-4 border-t border-zinc-800 dark:border-zinc-200">
                            <span className="text-sm font-black uppercase tracking-widest">Total</span>
                            <div className="flex gap-4 text-lg">
                                <span className="text-purple-500 font-black flex items-center gap-1.5"><motion.span>{displayedXp}</motion.span> <Star size={18}/></span>
                                <span className="text-yellow-500 font-black flex items-center gap-1.5"><motion.span>{displayedGold}</motion.span> <Coins size={18}/></span>
                            </div>
                        </div>
                    </div>
                )}
