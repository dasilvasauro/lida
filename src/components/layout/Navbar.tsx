import { CheckCircle2, Repeat, ShoppingBag, User, NotebookPen } from 'lucide-react';
...
export const Navbar = ({ currentTab, setCurrentTab }: NavbarProps) => {
    const tabs = [
        ...
        { id: 'notes', icon: NotebookPen, label: 'Notas' }, // <-- NOVO BOTÃO
    ...
    return (
        <div className="fixed bottom-6 left-1/2 -translate-x-1/2 z-50 w-[95%] max-w-[400px] flex justify-center">
            <div className="flex items-center justify-between w-full gap-1 px-3 py-3 bg-white/80 dark:bg-zinc-900/80 backdrop-blur-xl border border-zinc-200 dark:border-zinc-800 rounded-full shadow-[0_8px_30px_rgba(0,0,0,0.08)] dark:shadow-[0_8px_30px_rgba(0,0,0,0.5)]">
            {tabs.map((tab) => {
                const Icon = tab.icon;
